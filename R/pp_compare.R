...
#'
#' @param x An object of class \code{sf} or \code{data.frame}
#'     including estimated and actual values
...
pp_compare <- function(x, estimated, actual, title) {
  ...
  rmse <- Metrics::rmse(x[, actual, drop = T], x[, estimated,
                                                 drop = T])
  mae <- Metrics::mae(x[, actual, drop = T], x[, estimated,
                                               drop = T])
  linear_model <- lm(x[, actual, drop = T] ~ x[, estimated,
                                               drop = T])
  correlation_coef <- round(summary(linear_model)$r.squared, 3)
  myList <- list(rmse = rmse, mae = mae, linear_model = linear_model,
                 correlation_coef = correlation_coef)
  plot(x[, actual, drop = T], x[, estimated, drop = T], col = "#634B56",
       main = substitute(paste(title, ", R"^2 ~ paste("= ",
                                                      correlation_coef)), list(title = title, correlation_coef = correlation_coef)),
       cex.main = 1.2, xlab = "Actual", ylab = "Estimated")
